...
// ─── словари ─────────────────────────────────────────────────────────────────
const NODE_PREFIXES = [
  'Сервис', 'Модуль', 'Подсистема', 'Шлюз', 'Адаптер', 'Реестр',
  'Хранилище', 'Процессор', 'Агент', 'Брокер', 'Планировщик', 'Валидатор',
  'Координатор', 'Диспетчер', 'Маршрутизатор', 'Синхронизатор',
];
const NODE_SUBJECTS = [
  'Авторизации', 'Данных', 'Метаданных', 'Пользователей', 'Отчётов',
  'Аналитики', 'Мониторинга', 'Конфигурации', 'Уведомлений', 'Интеграции',
  'Платежей', 'Заказов', 'Справочников', 'Документов', 'Аудита',
  'Доступа', 'Безопасности', 'Контента', 'Событий', 'Процессов',
  'Ресурсов', 'Сессий', 'Токенов', 'Запросов', 'Задач', 'Правил',
  'Шаблонов', 'Потоков', 'Кэша', 'Логов',
];
const NODE_SUFFIXES = ['', '', '', '', 'API', 'v2', 'Core', 'Lite', 'Hub', 'Engine'];
const ISLAND_NAMES = [
  'Безопасности', 'Данных', 'Платформы', 'Пользователей', 'Аналитики',
  'Интеграции', 'Инфраструктуры', 'Бизнес-логики', 'Представления',
  'Хранения', 'Отчётности', 'Мониторинга', 'Обработки', 'Управления',
  'Авторизации', 'Заказов', 'Платежей', 'Конфигурации', 'Документооборота',
  'Метаданных',
];
const EDGE_TITLES = [
  'Предоставляет данные', 'Использует', 'Зависит от', 'Поставляет',
  'Читает из', 'Записывает в', 'Уведомляет', 'Оркестрирует',
  'Синхронизирует', 'Подписывается на', 'Публикует в', 'Делегирует',
];
const COLORS = [
  '#3b82f6', '#8b5cf6', '#0ea5e9', '#ef4444', '#06b6d4', '#f59e0b',
  '#22c55e', '#ec4899', '#f97316', '#a855f7', '#14b8a6', '#64748b',
];
const ISLAND_COLORS = {
...
const SHORT_DESCS = [
  'Основной сервис домена', 'Вспомогательный компонент', 'Агрегатор данных',
  'Точка интеграции', 'Шлюз для внешних систем', 'Хранилище основных данных',
  'Обработчик событий', 'Оркестратор процессов', 'Реестр конфигураций',
  'Брокер сообщений', 'Валидатор бизнес-правил', 'Процессор транзакций',
];
const EDGE_SHORT_DESCS = [
  'Синхронный вызов', 'Асинхронный поток', 'Реактивная подписка',
  'Пакетная выгрузка', 'gRPC-интеграция', 'REST API', 'Kafka-топик',
];
const ISLAND_SHORT_DESCS = [
  'Ключевой домен системы', 'Изолированный контур', 'Функциональная группа',
  'Зона ответственности', 'Сервисный кластер',
];
...
const nodes = Array.from({ length: NODE_COUNT }, (_, i) => {
  ...
  const r = rng();
  const nodeIslandIds = r < 0.60 ? [pick(islandIds)]
    : r < 0.75 ? pickN(islandIds, 2)
    : [];

  const extra = { color };
  if (rng() < 0.15) extra.background = `${color}10`;
  if (rng() < 0.10) extra.borderColor = color;
  if (rng() < 0.10) extra.titleColor = color;
...
console.log(`Узлов: ${nodes.length}, рёбер: ${edges.length}, островов: ${islands.length}`);
...
const layoutOptions = NODE_COUNT <= 300
  ? {
...
console.log('');
console.log('Теперь в src/GraphView.tsx раскомментируй "Вариант 2" и закомментируй "Вариант 1".');
