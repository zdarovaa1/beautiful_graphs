...
const PreviewExportBridge = forwardRef<PreviewExportHandle, PreviewExportBridgeProps>(function PreviewExportBridge(
  { containerRef },
  ref,
) {
  const { fitView } = useReactFlow()
...
const PreviewCanvas = memo(function PreviewCanvas({
  ...
  return (
    <div className={`${styles.previewWrap} ${transparent ? styles.previewWrapTransparent : ''}`} ref={flowRef}>
    ...
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.dialog} onClick={(e) => e.stopPropagation()}>
        <header className={styles.header}>
          <h2 id='screenshot-preview-title' className={styles.title}>
            {texts.screenshotModal.title}
          </h2>
          <Tooltip title={texts.screenshotModal.close}>
        ...
        <footer className={styles.footer}>
          <span className={styles.hint}>
            {counts.graphs} {texts.screenshotModal.hintNodes}, {counts.islands} {texts.screenshotModal.hintIslands},{' '}
            {edges.length} {texts.screenshotModal.hintEdges}
          </span>
          <label className={styles.bgToggle}>
            <input type='checkbox' checked={transparent} onChange={(e) => setTransparent(e.target.checked)} />
            {texts.screenshotModal.transparentBg}
          </label>
          <button type='button' className={styles.btn} onClick={onClose}>
            {texts.screenshotModal.cancel}
          </button>
          <button type='button' className={styles.btnPrimary} disabled={!!saving} onClick={() => save('png')}>
            {saving === 'png' ? texts.screenshotModal.saving : texts.screenshotModal.savePng}
          </button>
          <button
            type='button'
            ...
            onClick={() => save('jpeg')}
          >
            {saving === 'jpeg' ? texts.screenshotModal.saving : texts.screenshotModal.saveJpeg}
          </button>
          <button
            type='button'
            className={styles.btnPrimary}
            ...
            onClick={() => save('webp')}
          >
            {saving === 'webp' ? texts.screenshotModal.saving : texts.screenshotModal.saveWebp}
          </button>
        </footer>
      </div>
    </div>,
