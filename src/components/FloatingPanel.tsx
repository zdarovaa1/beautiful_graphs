import { createContext, memo, useCallback, useEffect, useMemo, useRef, useState, type CSSProperties, type ReactNode } from 'react';
...
/** Контекст: кнопки Pin + Свернуть + collapse/expand для кнопки закрытия */
...
export type SnapEdge =
  ...
  | null;
export interface PanelSize { w: number; h: number; left: number; top: number }
...
/**
 * Размеры панели при snap и pin — настраивайте здесь.
 *
 * SNAP_WIDTH  — начальная ширина для snap слева / справа и в углах
 * SNAP_HEIGHT — начальная высота для snap сверху / снизу и в углах
 *
 * Полная высота (left/right) и полная ширина (top/bottom) — через CSS
 * (top+bottom / left+right = 100% контейнера), без window.innerHeight.
 */
export const SNAP_WIDTH = 360;
export const SNAP_HEIGHT = 420;
/** Высота угловых snap — доля контейнера */
export const SNAP_CORNER_HEIGHT = '50%';

/** Угловой snap (top-left, bottom-right, …) */
...
/** Стили позиционирования при snap (слот из SnapLayout или дефолтные якоря) */
...
/** Начальные размеры rect при snap (позиция задаётся CSS-якорями) */
function applySnap(snap: SnapEdge, r: Rect, maxW: number, _margin = SNAP_MARGIN): Rect {
  ...
  switch (snap) {
    case 'left':         return { ...r, w: sw };
    case 'right':        return { ...r, w: sw };
    case 'top':          return { ...r, h: sh };
    case 'bottom':       return { ...r, h: sh };
    case 'top-left':     return { ...r, w: sw };
    case 'top-right':    return { ...r, w: sw };
    case 'bottom-left':  return { ...r, w: sw };
    case 'bottom-right': return { ...r, w: sw };
    default: return r;
  }
}
...
function detectSnap(x: number, y: number, w: number, h: number): SnapEdge {
  ...
  if (nB && nL) return 'bottom-left'; if (nB && nR) return 'bottom-right';
  if (nL) return 'left'; if (nR) return 'right';
  // snap сверху/снизу на всю ширину — отключено
  // if (nT) return 'top';
  // if (nB) return 'bottom';
  return null;
}
...
/**
 * Стороны ресайза, прижатые к краю экрана — ручки скрыты, snap и pin сохраняются.
 * Пример: snap=left → ресайз только справа (r, br, tr).
 */
...
function load(key: string | undefined, fallback: Rect, minW: number, minH: number, maxW: number): Rect {
  ...
  } catch { return clampRect(fallback, minW, minH, maxW); }
}
...
export const FloatingPanel = memo(function FloatingPanel({
  ...
  minWidth = 260, minHeight = 220, zIndex = 7,
  storageKey, title, onLayout, children,
}: FloatingPanelProps) {
  ...
  // snapEdge обновляется ТОЛЬКО при явном drag→snap — не через detectSnap от позиции.
  // Это гарантирует что Pin-кнопка появляется только после осознанного действия юзера.
  ...
  // Рефы для синхронного доступа в колбэках без пересоздания
  ...
  // Последние позиции rect во время drag/resize — нужны в stop() до рендера
  ...
  // Сохраняем rect и snap при каждом изменении
  useEffect(() => {
  ...
  // При размонтировании — освобождаем canvas
  useEffect(() => {
    return () => { onLayoutRef.current?.(null, null, storageKeyRef.current ?? ''); };
  }, []);
  ...
  // Позиция и размер панели → SnapLayout (toolbar + snap-слоты)
  useEffect(() => {
    const el = panelRef.current;
    const key = storageKeyRef.current ?? '';
    if (!el || collapsed) {
  ...
  const onMove = useCallback((e: PointerEvent) => {
    ...
    if (dir.includes('t')) { const nh = Math.max(minHeight, s.h - dy); y = Math.max(0, s.y + (s.h - nh)); h = nh; }
  ...
  const begin = useCallback((e: React.PointerEvent, m: Exclude<Mode, null>) => {
    mode.current = m;
  ...
  /** Прикрепить / открепить: визуально фиксирует панель на стороне */
  const togglePin = useCallback(() => {
    if (!snapRef.current) return;
    const next = !pinnedRef.current;
    setPinned(next);
    pinnedRef.current = next;
    // Пересчёт snap-слотов с новыми отступами
    const el = panelRef.current;
    const key = storageKeyRef.current ?? '';
  ...
  /** Свернуть / развернуть:
   *  - Сворачивание: уведомляем layout (canvas освобождается), рендерим mini-bar внизу
   *  - Разворачивание: восстанавливаем snap если был, уведомляем layout */
  ...
  // Кнопки управления панелью — передаются в заголовок дочернего компонента через контекст
  ...
  const rh = (dir: ResizeDir) => {
    ...
    return (
      <div
        ...
        onPointerDown={(e) => { e.stopPropagation(); begin(e, `resize-${dir}`); }}
      />
    );
  };

  // CSS-классы для pinned-состояния (убираем скругление на прикреплённых сторонах)
  const panelClass = [
    styles.panel,
  ...
  return (
    <>
      {ghostStyle && (
        <div className={styles.snapGhost} style={ghostStyle} />
      )}
      <div
        ref={panelRef}
      ...
      >
        <FloatingPanelActionsContext.Provider value={panelActions}>
          {children}
        </FloatingPanelActionsContext.Provider>
