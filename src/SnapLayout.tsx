...
const DEFAULT_TOOLBAR_LAYOUT: ToolbarLayoutOptions = { showFilterStats: false }
...
function toolbarBounds(left: number, top: number, layout: ToolbarLayout, showFilterStats: boolean) {
...
function assignSideSlots(snaps: Map<string, PanelInfo>, side: 'left' | 'right', out: Map<string, SideSlot>) {
  ...
  for (let ci = 0; ci < cols.length; ci++) {
    ...
    const anchor: SideSlot = side === 'left' ? { left: offset, width: col.w } : { right: offset, width: col.w }
    const cornerPinned = (key: string) => isPinned(key)
    const cornerH = (key: string) => (cornerPinned(key) ? '50%' : `calc(50% - ${m * 1.5}px)`)
...
function computeToolbarPosition(panels: Map<string, PanelInfo>, showFilterStats = false): ToolbarPosition {
...
interface SnapLayoutCtx {
  ...
  reportToolbarLayout: (options: ToolbarLayoutOptions) => void
}
...
export function useToolbarPosition() {
  return useContext(Ctx).toolbarPos
}
export function useReportToolbarLayout() {
  return useContext(Ctx).reportToolbarLayout
}
export function useRegisteredPanels() {
  return useContext(Ctx).panels
}
...
export function usePanelStack(storageKey: string | undefined) {
  const { panelStack, bringPanelToFront, registerPanelStack, unregisterPanelStack } = useContext(Ctx)
  useEffect(() => {
...
export function SnapLayoutProvider({ children }: { children: ReactNode }) {
  ...
  const [toolbarPos, setToolbarPos] = useState<ToolbarPosition>(() =>
    computeToolbarPosition(new Map(), DEFAULT_TOOLBAR_LAYOUT.showFilterStats),
  )
  const panelsRef = useRef(panels)
  panelsRef.current = panels
  const toolbarLayoutRef = useRef<ToolbarLayoutOptions>(DEFAULT_TOOLBAR_LAYOUT)
  ...
  const reportToolbarLayout = useCallback((options: ToolbarLayoutOptions) => {
    const prev = toolbarLayoutRef.current
    if (prev.showFilterStats === options.showFilterStats) return
    toolbarLayoutRef.current = options
    setToolbarPos(computeToolbarPosition(panelsRef.current, options.showFilterStats))
  }, [])

  const reportLayout = useCallback((snap: SnapEdge, size: PanelSize | null, key: string, pinned = false) => {
    setPanels((prev) => {
      ...
      setSnapSlots(slots)
      setToolbarPos(computeToolbarPosition(next, toolbarLayoutRef.current.showFilterStats))
      return next
    })
  }, [])
  const value = useMemo(
