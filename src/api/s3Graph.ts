 ...
 */
export async function fetchGraphFromS3({
  key,
  url,
  signal,
}: FetchGraphFromS3Options): Promise<GraphLayoutBundle> {
 ...
 */
export async function uploadGraphToS3(
  url: string,
  bundle: GraphLayoutBundle,
  signal?: AbortSignal,
): Promise<void> {
  ...
  if (!response.ok) {
    throw new S3GraphFetchError(
      `Не удалось сохранить граф: ${response.status} ${response.statusText}`,
      response.status,
    );
  }
}
