...
/** Доп. отступ на каждый уровень вложенности (когда меньший остров "внутри" большего). */
...
export function buildInitialSettings(
  ...
  const edgeTypeMap = Object.fromEntries(
    [...new Set(edgeDefs.map((e) => e.type))].map((t) => [t, true]),
  ) as Record<EdgeType, boolean>;
  const islandTypeMap = Object.fromEntries(
...
/**
 * Вычисляет границы острова с учётом смещения для вложенности.
 * extraPad добавляется ко всем сторонам — увеличивает остров, чтобы
 * заголовок меньшего острова не накладывался на заголовок большего.
 */
...
export function computeIslandNodes(
  ...
  const visibleIslands = islandDefs.filter(
    (i) => settings.islandTypes[i.type] && settings.islandNames[i.id],
  );

  // Предвычисляем множества видимых членов каждого острова
  ...
  for (const island of visibleIslands) {
    memberSets.set(
      ...
      new Set(
        nodeDefs
          .filter((n) => n.islandIds.includes(island.id) && visibleNodeIds.has(n.id))
          .map((n) => n.id),
      ),
    );
  }
  ...
  for (const island of visibleIslands) {
    ...
    // Уровень вложенности: сколько МЕНЬШИХ островов делят с нами хотя бы один узел.
    // Для каждого такого острова мы добавляем ISLAND_NEST_OFFSET — визуально
    // становимся «внешним» островом, и наш заголовок оказывается выше.
    let nestLevel = 0;
    ...
    const bounds = islandBoundsWithPad(
      myMembers, nodeDefs, positions, savedPositions, nestLevel * ISLAND_NEST_OFFSET,
    );
    if (!bounds) continue;
...
export function computeEdges(
  ...
  return edgeDefs
    .filter((e) => settings.edgeTypes[e.type] && visibleNodeIds.has(e.source) && visibleNodeIds.has(e.target))
    .map((e) => ({
      id: e.id,
      source: e.source,
      target: e.target,
...
/** Смещает выбранные узлы по оси, сохраняет результат в savedPositions и React Flow. */
export function applyAlignment(
  ...
  setSavedPositions((prev) => {
    const next = { ...prev };
    newPos.forEach((pos, id) => { next[id] = pos; });
    return next;
  });
  setNodes((ns) => ns.map((n) => { const p = newPos.get(n.id); return p ? { ...n, position: p } : n; }));
}
...
/** ELK-укладка выделенных узлов с сохранением центра выделения */
